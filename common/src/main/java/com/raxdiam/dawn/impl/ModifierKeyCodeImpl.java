...
import com.raxdiam.dawn.api.Modifier;
...
public class ModifierKeyCodeImpl implements ModifierKeyCode {
    ...
    public Component getLocalizedName() {
        ...
        if (modifier.hasShift())
            base = Component.translatable("modifier.dawn-config.shift", base);
        if (modifier.hasControl())
            base = Component.translatable("modifier.dawn-config.ctrl", base);
        if (modifier.hasAlt())
            base = Component.translatable("modifier.dawn-config.alt", base);
        return base;
    }

...
public abstract class AbstractConfigListEntry<T> extends AbstractConfigEntry<T> {
    ...
    public void render(GuiGraphics graphics, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
        if (isMouseInside(mouseX, mouseY, x, y, entryWidth, entryHeight)) {
            ...
            if (getParent() instanceof DawnConfigScreen.ListWidget)
                ((DawnConfigScreen.ListWidget<AbstractConfigEntry<T>>) getParent()).thisTimeTarget = new Rectangle(area.x, area.y + getParent().getScroll(), area.width, area.height);
        }
    }
